...
struct ReadAnnotationInfo
{
  ...
  // whether the annotatino is trusted.
  bool trusted;

  ReadAnnotationInfo() : where(NULL), text(NULL), trusted(false) {}

  ReadAnnotationInfo(String *_where, String *_text, bool _trusted)
    : where(_where), text(_text), trusted(_trusted)
  {}
};
...
extern "C" void XIL_ReadAnnotationFile(const char *file)
{
  ...
  // name
  // email
  // hook (as generated by Where::PrintHook).
  // text
  // trusted
  ...
  for (size_t ind = 0; ind < strings.Size(); ind++) {
    ...
    char *text = strings[++ind];
    if (*text == 0) continue;

    char *trusted = strings[++ind];
    if (*trusted == 0) continue;

    String *new_text = String::Make(text);
    ...
    while (true) {
      ...
      *space = 0;
      ...
      ReadAnnotationInfo info(where, new_text, new_trusted);
